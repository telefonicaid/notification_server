...
var SimplePushAPI_v1 = function() {
  this.processRequest = function(request, body, response) {
    ...
    if (URI.length < 3) {
      ...
      log.debug('NS_UA_SimplePush_v1::processMozRequest --> Not enough path');
      return;
    }
    ...
    if (URI[1] !== kMozASFrontendVersion) {
      ...
      log.debug('NS_UA_SimplePush_v1::processMozRequest --> Version not supported, received: ' + URI[1]);
      return;
    }
    ...
    if (URI[2] !== 'notify') {
      ...
      log.debug('NS_UA_SimplePush_v1::processMozRequest --> API call not known, received: ' + URI[2]);
      return;
    }
    ...
    if (versions[0] !== 'version') {
      ...
      log.debug('NS_UA_SimplePush_v1::processMozRequest --> Bad body, received lhs: ' + versions[0]);
      return;
    }
    ...
    if (!isVersion(version)) {
      ...
      log.debug('NS_UA_SimplePush_v1::processMozRequest --> Bad version, received rhs: ' + version);
      return;
    }
