...
function onNewPushMessage(notification, apptoken, callback) {
  ...
  try {
    ...
    log.notify('NS_AS::onNewPushMessage --> Rejected. Not valid JSON notification');
  ...
  if (normalizedNotification.messageType != "notification") {
    log.notify('NS_AS::onNewPushMessage --> Rejected. Not valid messageType');
    return callback('{"status":"ERROR", "reason":"Not messageType=notification"}', 400);
  }

  //If not signed, reject
  if (!json.signature) {
    log.notify('NS_AS::onNewPushMessage --> Rejected. Not signed');
    return callback('{"status":"ERROR", "reason":"Not signed"}', 400);
  }

  //If not id, reject
  if (!normalizedNotification.id) {
    log.notify('NS_AS::onNewPushMessage --> Rejected. Not id');
  ...
  if ((normalizedNotification.message.length > consts.MAX_PAYLOAD_SIZE) ||
    ...
    log.notify('NS_AS::onNewPushMessage --> Rejected. Notification with a big body (' + normalizedNotification.message.length + '>' + consts.MAX_PAYLOAD_SIZE + 'bytes), rejecting');
  ...
  dataStore.getPbkApplication(apptoken, function(pbkbase64) {
    ...
    if (!crypto.verifySignature(normalizedNotification.message, json.signature, pbk)) {
      log.notify('NS_AS::onNewPushMessage --> Rejected. Bad signature, dropping notification');
      return callback('{"status":"ERROR", "reason":"Bad signature, dropping notification"}', 400);
    }

    var id = uuid.v1();
    log.notify("NS_AS::onNewPushMessage --> Storing message '" + JSON.stringify(normalizedNotification) + "' for the '" + apptoken + "'' apptoken. Internal Id: " + id);
...
server.prototype = {
  ...
  onHTTPMessage: function(request, response) {
    ...
    case 'notify':
      ...
      log.notify("NS_AS::onHTTPMessage --> Notification for " + url.token);
