...
/**
 * Strict disabled by octal numbers for file permissions
 */
//'use strict';
...
Logger.prototype = {
    ...
    init: function(logfile, appname, consoleOutput) {
        try {
            var stats = fs.lstatSync(logparams.BASE_PATH);
            if (!stats.isDirectory()) {
                console.error(logparams.BASE_PATH + 'is not a directory');
                process.exit(1);
                return;
            }
        } catch (e) {
        ...
        this.logfilealarm = fs.createWriteStream(logparams.ALARM, {
            flags: 'a',
            encoding: null,
            mode: 0644
        });
        ...
        this.log('START', '---------8<---------8<---------8<---------8<---------8<---------8<---------8<---------', false);
